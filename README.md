...
- [부록 A](#부록-a)
  - [구 버전의 인터넷 익스플로러 지원하기](#구-버전의-인터넷-익스플로러-지원하기)
    - [인터넷 익스플로러 개요](#인터넷-익스플로러-개요)
    - [최신 자바스크립트 코드를 인터넷 익스플로러에서 실행하기](#최신-자바스크립트-코드를-인터넷-익스플로러에서-실행하기)
      - [바벨: 트랜스 컴파일러](#바벨-트랜스-컴파일러)
      - [es-shims](#es-shims)
...
**인터넷 익스플로러**는 마이크로 소프트에서 과거에 개발했던 웹브라우저로 더 이상 업데이트 되지 않고 있습니다. 웹 브라우저가 업데이트 되지 않는 관계로 최신 자바스크립트 기능을 사요할 수 없습니다.

전 세계적으로 점유율이 1% 이하이지만, 국내에서는 Active X 등의 요인으로 아직 높은 점유율을 가지고 있습니다.

### 최신 자바스크립트 코드를 인터넷 익스플로러에서 실행하기

최신 자바스크립트 코드를 100% 인터넷 익스플로러에서 실행하게 할 수는 없습니다. 따라서 최신 유튜브와 같은 거대한 사이드들은 인터넷 익스플로러를 포기합니다.

하지만 자바스크립트가 많이 활용되지 않는 간단한 쇼핑몰 사이트에서는 최신 자바스크립트를 운용할 수 있도록 만들 수 있습니다.

#### 바벨: 트랜스 컴파일러

**바벨**(babel)은 최신 버전의 자바스크립트로 작성된 코드를 구 버전에서 작동하게 만들어주는 변환기 입니다.

이러한 변환기를 **트랜스 컴파일러**, **트랜스파일러**라고 합니다.

바벨 REPL(https://babeljs.io/repl)을 이용하여

최신 자바스크립트 코드를 작성하면 구 버전에서 사용할 수 있는 형태로 변경해줍니다.

#### es-shims

하지만 문법적으로 변환은 해주어도 기능적인 변환은 해주지 않습니다. 예를 들어 배열의 filter(), forEach() 메소드 등은 구 버전의 인터넷 익스플로러에서 동작하지 않습니다.

이러한 메소드 등의 기능을 추가해주는 것을 **폴리필**(Polyfill)이라고 합니다.

대표적인 예시로 **Modernizr**, es-**shims**, **polyfill.io**등이 있습니다.

es-shims github를 방문하면 CDN을 활용하여 구 버전의 인터넷 익스플로러에서 어느 정도까지 최신 기능을 지원하는지 확인할 수 있습니다.
